...
fragment float4 fragment_main(VertexOut in [[stage_in]],
  ...
  return in.color * clamp(float4(in.position[0] / size[0],
                                 in.position[1] / size[0], 0.0, 1.0),
                          0.3, 1.0);
}
...
kernel void game(constant bool *in [[buffer(0)]],
  ...
  uint alive = in[index - 1] + in[index + 1] + in[index - size] +
               in[index + size] + in[index - 1 - size] + in[index - 1 + size] +
